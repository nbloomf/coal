...
\end{verbatim}
where the only new argument, \texttt{range-selector}, is a pair of cell names separated by a colon, like \texttt{A3:A5} or \texttt{B1:D4}. Note that it only makes sense to interpret a range selector argument if we also have a sheet selector. So passing in a \texttt{range-selector} without a \texttt{sheet-ID} has no effect.
